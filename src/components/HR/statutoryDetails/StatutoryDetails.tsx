// src/components/HR/statutoryDetails/StatutoryDetails.tsx
...
import { StatutoryDetailsProps, StatutoryField, DropdownOption, StatutoryDetailsRenderProps } from './StatutoryDetails.types';
import { useStatutoryDetails } from './useStatutoryDetails';

// --- Presentational Component (Inline for Simplicity & Line Count) ---
const StatutoryDetailsRender: React.FC<StatutoryDetailsRenderProps> = ({
  ...
  return (
    <>
      {/* Separator + Statutory Details UI */}
      <div className="border-t pt-4">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Statutory Details</h3>
        ...
        <div className="flex space-x-2">
          <button type="button" onClick={onShowAdd} className="flex items-center px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
            <Plus className="w-4 h-4 mr-2" /> Add Statutory Field
          </button>
      ...
      {optionsModalField && (
        <OptionsModal
          title={`Options for ${statutoryFields.find((f) => f.field_id === optionsModalField)?.field_name || optionsModalField}`}
...
// --- Main Component ---
export const StatutoryDetails: React.FC<StatutoryDetailsProps> = (props) => {
  const state = useStatutoryDetails(props);
  ...
  const renderProps: StatutoryDetailsRenderProps = {
    statutoryFields: state.statutoryFields,
  ...
  return <StatutoryDetailsRender {...renderProps} />;
};

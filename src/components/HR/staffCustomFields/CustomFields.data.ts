// src/components/HR/staffCustomFields/CustomFields.data.ts
// @tag: Functions for fetching and modifying Custom Fields data in Supabase for Staff.
...
export const deleteFieldFromDb = async (fieldId: string): Promise<void> => {
  const { error } = await supabase.from('staff_custom_fields').delete().eq('field_id', fieldId);
  if (error) {

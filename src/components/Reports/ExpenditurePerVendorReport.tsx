...
interface ExpenditurePerVendorReportProps {
  onClose: () => void;
}
...
interface PageData {
  records: VendorAggregate[];
  pageNumber: number;
  totalPages: number;
  showSummary: boolean;
}

export const ExpenditurePerVendorReport: React.FC<ExpenditurePerVendorReportProps> = ({ onClose }) => {
  const [showConfigPopup, setShowConfigPopup] = useState(true);
  const [dateFrom, setDateFrom] = useState<string>('');
  const [dateTo, setDateTo] = useState<string>('');
  const [vendors, setVendors] = useState<VendorAggregate[]>([]);
  ...
  const splitIntoPages = useCallback((items: VendorAggregate[]): PageData[] => {
    if (items.length === 0) return [];
    
    const pages: PageData[] = [];
    let currentPageRecords: VendorAggregate[] = [];
  ...
  useEffect(() => {
    if (vendors.length > 0) {
      const splitPages = splitIntoPages(vendors);
      setPages(splitPages);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const { data: expenses, error } = await supabase
        .from('expenses_view')
        .select('vendor_name, amount, payment_status, expense_date, date_paid')
      ...
      // Aggregate by vendor
      const aggregated: { [key: string]: VendorAggregate } = {};
      
      (expenses || []).forEach(expense => {
        const vendorName = expense.vendor_name || 'Unknown Vendor';
        ...
        aggregated[vendorName].totalAmount += amount;
        aggregated[vendorName].expenseCount += 1;
        if (isPaid) {
          aggregated[vendorName].paidAmount += amount;
        } else {
          aggregated[vendorName].outstandingBalance += amount;
        }
        
        // Update last expense date if this is more recent
        if (expense.expense_date > aggregated[vendorName].lastExpenseDate) {
          aggregated[vendorName].lastExpenseDate = expense.expense_date;
        }
      });

      // Sort by total amount descending
      const sortedVendors = Object.values(aggregated).sort((a, b) => b.totalAmount - a.totalAmount);
  ...
  const handleExportToPdf = useCallback(async () => {
    ...
    try {
      ...
      const filename = `Expenditure_by_Vendor_${dateFrom}_to_${dateTo}.pdf`;
      pdf.save(filename);
    } catch (error) {
  ...
  const getSummaryStats = () => {
    const totalExpenditure = vendors.reduce((sum, vendor) => sum + vendor.totalAmount, 0);
    ...
    return { totalExpenditure, totalPaid, totalOutstanding, totalVendors };
  };
        ...
        <div className="bg-white shadow-xl w-full max-w-md my-8">
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold text-gray-800">Expenditure Per Vendor</h2>
                  ...
                  >
                    ...
                    <div className="mb-4 border-b border-gray-200 pb-3 flex-shrink-0">
                      <div className="flex justify-between items-start">
                        <div>
                          <h1 className="text-3xl font-normal text-gray-900 mb-2">Expenditure Per Vendor</h1>
                          <div className="text-sm text-gray-600 space-y-1">
                            <p><strong>Description:</strong> Total expenditure breakdown by vendors</p>
                          ...
                          <table className="w-full border-collapse">
                            <thead>
                              <tr className="bg-gray-50 border-b-2 border-gray-200">
                                ...
                                <th className="text-right p-3 text-sm font-semibold text-gray-700" style={{ width: '10%' }}>Count</th>
                                <th className="text-right p-3 text-sm font-semibold text-gray-700" style={{ width: '18%' }}>Paid (Ksh)</th>
                                <th className="text-right p-3 text-sm font-semibold text-gray-700" style={{ width: '18%' }}>Outstanding (Ksh)</th>
                                <th className="text-right p-3 text-sm font-semibold text-gray-700" style={{ width: '18%' }}>Total (Ksh)</th>
                                <th className="text-left p-3 text-sm font-semibold text-gray-700" style={{ width: '12%' }}>Last Date</th>
                              </tr>
                            </thead>
                            <tbody>
                              {pageData.records.map((vendor, index) => (
                                <tr key={vendor.vendorName} className={`border-b border-gray-100 ${index % 2 === 0 ? 'bg-white' : ''}`} style={index % 2 === 1 ? { backgroundColor: '#fcfcfd' } : {}}>
                                  <td className="p-3 text-sm text-gray-600">{previousPagesItemsCount + index + 1}</td>
                                  <td className="p-3 text-sm text-gray-900 font-medium">{vendor.vendorName}</td>
                                  <td className="p-3 text-sm text-gray-600 text-right">{vendor.expenseCount.toLocaleString()}</td>
                                  <td className="p-3 text-sm text-right text-green-600">{formatCurrency(vendor.paidAmount)}</td>
                                  <td className="p-3 text-sm text-right text-orange-600">{formatCurrency(vendor.outstandingBalance)}</td>
                                  <td className="p-3 text-sm text-right font-medium text-red-600">{formatCurrency(vendor.totalAmount)}</td>
                                  <td className="p-3 text-sm text-gray-600">{formatDate(vendor.lastExpenseDate)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                    ...
                    {pageData.showSummary && (
                      <div className="mt-8 grid grid-cols-3 gap-4 flex-shrink-0">
                        ...
                        <div className="bg-gray-50 p-4 rounded-lg">
                          ...
                          <p className="text-2xl font-semibold text-orange-600">{formatCurrency(stats.totalOutstanding)}</p>
                        </div>
                      </div>
                    )}
...
export default ExpenditurePerVendorReport;
